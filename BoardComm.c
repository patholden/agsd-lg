...
#ifdef ZDEBUG
syslog(LOG_DEBUG, "DLevel614 curXY %x %x ctrl %x delXY %x %x nPts %d index %x"
                , lg_sensor.xy_curpt.xdata
                , lg_sensor.xy_curpt.ydata
                , lg_sensor.xy_curpt.ctrl_flags
                , lg_sensor.xy_delta.xdata
                , lg_sensor.xy_delta.ydata
                , lg_sensor.nPoints
                , lg_sensor.start_index
                );
#endif
...
#ifdef ZDEBUG
syslog(LOG_ERR, "DLevel634 read num %d try %d  timeout %d  points %d", (int)num, j, time_out, nPoints );
#endif
        usleep(time_out);
        time_out *= 2;
      }
    ...
    for (i = 0; i < nPoints; i++)
      {
syslog(LOG_ERR, "DLevel read  i %d  c_out %d n %d", i, c_out[i], nPoints );
      }
#endif
    sense_count = 0;
    if (num > 0)

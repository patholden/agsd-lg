...
ParseVisionFocus(int length, char * infile)
{
	 ...
	 // no periods allowed
	 if ( *ptr == 0x2E ) {
	   syslog(LOG_ERR, "ParseVisionFocus error: period at %d", count);
	   return( -1 );
	 }
	 ...
	 if ( strncmp( ptr, testStr, strlen(testStr )) == 0 && tDone == 0 ) {
	   ...
	   if ( itest != 1 ) { 
             syslog(LOG_ERR, "ParseVisionFocus error: %s sscanf at %d  itest %d",
		    testStr, count, itest);
             return(-1);
	   }
	   ...
	   for ( i=0; i < tmplen; i++ ) {
	     if ( ! isdigit( number1[i] ) ) {
	       syslog(LOG_ERR, "ParseVisionFocus error: %s isdigit %d  number1 %c ",
		      testStr, i, number1[i]);
	       return(-1);
	     }
	   }
	 ...
	 if ( strncmp( ptr, testStr, strlen(testStr )) == 0 && tDone == 2 ) {
	   ...
	   if (strncmp(shd, "cw", 2) == 0)
	     ...
	     {
	       syslog(LOG_ERR, "ParseVisionFocus error: %s sscanf at %d  not CW or CCW",
		      testStr, count);
	       return(-1);
	     }
	   tDone = 3;
	 ...
	 if ( strncmp(ptr, testStr, strlen(testStr )) == 0 && tDone == 3 ) {
	   ...
	   if (itest != 1)
	     { 
	       syslog(LOG_ERR, "ParseVisionFocus error: %s sscanf at %d  itest %d seekdir %d",
	 ...
	 if ( tDone>=4 && ptr[0]==0x0D && ptr[1]==0x0A && (count+2)<length ) {
	   ...
	   if (itest != 2)
	     { 
	       syslog(LOG_ERR, "ParseVisionFocus error: distance sscanf at %d  itest %d",
		      count, itest);
	       return(-1);
	       ...
	       for (i=0; i < tmplen; i++)
		 {
		   if (!isdigit(number2[i]))
		     {
		       syslog(LOG_ERR,"ParseVisionFocus error: %s isdigit %d  number2 %c ",
			  testStr, i, number2[i]);
		       return(-1);
		     }
		 }
	       ...
	       if (i2 < lowlimit)
		 {
		   syslog(LOG_ERR, "ParseVisionFocus error: sscanf at %d i2 %d lowlimit %d",
			  count, i2, lowlimit);
		   return(-1); 
		 }
             if (i2 > highlimit)
	       {
		 syslog(LOG_ERR, "ParseVisionFocus error: sscanf at %d i2 %d highlimit %d",
			count, i2, highlimit);
		 return(-1);
	       } 
             if (i1 < minDistance)
	       {
		 syslog(LOG_ERR, "ParseVisionFocus error: sscanf at %d i1 %d minDistance %d",
			count, i1, minDistance);
		 return(-1);
	       }
             if (i1 > maxDistance)
	       {
		 syslog(LOG_ERR, "ParseVisionFocus error: sscanf at %d i1 %d maxDistance %d",
			count, i1, maxDistance);
		 return(-1);
	       }
             if (parsedDistance[i1] == 1)
	       {
                 syslog(LOG_ERR, "ParseVisionFocus error: sscanf at %d i1 %d duplicate parsedDistance",
			count, i1);
                 return(-1);
	       }
             parsedDistance[i1] = 1;
     ...
     if (tDone < 4)
       {
	 syslog(LOG_ERR, "ParseVisionFocus error: tDone %d", tDone);
	 return(-1);
       }
     return(0);
}
